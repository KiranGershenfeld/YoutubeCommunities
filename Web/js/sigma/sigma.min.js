...
sigma.classes.Cascade.call(this);var h=this;this.p={labelColor:"default",defaultLabelColor:"#000",labelHoverBGColor:"default",defaultHoverLabelBGColor:"#fff",
...
var slider_value = document.getElementById('label_slider_id').value
h.p.labelThreshold = 10 - slider_value;
for(var iter = 0; iter < a.nodes.length; iter++)
{
    //console.log(slider_value)
    if(a.nodes[iter].active == false && (!isLabelOnScreen(a.nodes[iter]) || a.nodes[iter].displaySize <= h.p.labelThreshold / 1))
    {
...
sorted_nodes = sorted_nodes.splice(-20);
...
this.task_drawEdge=function(){for(var b=a.edges.length,c,d,f=0;f++<h.p.edgesSpeed&&h.currentEdgeIndex<
...
b.content)})}var k=0,m={plugins:[]};
sigma.init=function(b){b=new F(b,(++k).toString());sigma.instances[k]=new x(b);return sigma.instances[k]};sigma.addPlugin=function(b,g,f){x.prototype[b]=g;m.plugins.push(f)};sigma.chronos=new function(){function b(a){window.setTimeout(a,0);return h}function g(){for(h.dispatch("frameinserted");n&&v.length&&f(););!n||!v.length?a():(B=(new Date).getTime(),m++,z=u-p,q=p-z,h.dispatch("insertframe"),b(g))}function f(){C%=v.length;if(!v[C].task()){var a=v[C].taskName;
