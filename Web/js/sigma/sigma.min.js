...
{
    if(!isLabelOnScreen(a.nodes[iter]) || a.nodes[iter].displaySize <= h.p.labelThreshold - 5)
    {
        a.nodes[iter].forceLabel = false;
    }
}
sorted_nodes = sorted_nodes.filter(function(element)
{
    return(isLabelOnScreen(element));
})
...
j=f;if(d.displaySize>=h.p.labelThreshold||d.forceLabel){
    var g="fixed"==h.p.labelSize?h.p.defaultLabelSize:h.p.labelSizeRatio*d.displaySize;j.font=h.p.fontStyle+g+"px "+h.p.font;j.fillStyle="node"==h.p.labelColor?d.color||h.p.defaultNodeColor:h.p.defaultLabelColor;j.fillText(d.label,Math.round(d.displayX+1.5*d.displaySize),Math.round(d.displayY+g/2-3));console.log(f.font)
}}else h.currentLabelIndex++;

return h.currentLabelIndex<b};this.task_drawEdge=function(){for(var b=a.edges.length,c,d,f=0;f++<h.p.edgesSpeed&&h.currentEdgeIndex<
